...
class TrainDiffusionUnetLowdimWorkspace:
    ...
    def run(self):
        ...
        # Make sure we have the monitor key from config
        monitor_key = cfg.checkpoint.topk.get("monitor_key", "train_loss")
        # Default to train_loss if not specified
        if monitor_key == "test_mean_score":
            print(f"Warning: monitor_key is set to 'test_mean_score' which is not available.")
            print(f"Setting monitor_key to 'train_loss' instead.")
            cfg.checkpoint.topk.monitor_key = "train_loss"
            monitor_key = "train_loss"
        
        # Print checkpoint config for debugging
        print(f"Checkpoint config: {OmegaConf.to_container(cfg.checkpoint)}")
        print(f"TopK config: {OmegaConf.to_container(cfg.checkpoint.topk)}")
        print(f"Using monitor_key: {monitor_key}")
